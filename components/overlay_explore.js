...
function krpanoplugin() {
    ...
    local.registerplugin = function (krpanointerface, pluginpath, pluginobject) {
        ...
        plugin.registerattribute("show_items_taketwo", show_items_taketwo);
    ...
    function show_video_menu(n) {
        ...
        if (videos[1])
        {
            console.log(videos[0], videos[1]);
            amp_player.src(videos[0], videos[1]);
            //amp_player.playlist(videos[0], videos[1]);
        }
        else
        {
    ...
    function show_item_lib(scene, floor_id) {
        ...
        for (var p in o) {
            ...
            items_div.forEach((item) => {
                if (x.items.includes(item.dataset.id)) {
                    
                    if (item.dataset.id != 'stairwell' && item.dataset.id != 'exit') {

                        item.innerHTML += "<a data-item='" + item.dataset.id + " ' data-label='" + p + "' class='item-lib-link'><img src='../"  +  app_version + "/skin/ak/hotspots/" + item.dataset.id + ".png' width='45' height='45' /></a>";
                    } else {
                    
                        if (item.dataset.id == 'stairwell' && x.stair) {
                            item.innerHTML += "<a style='padding:.5em;font-size:1.3em' data-label='" + p + "' data-item='stairwell' class='item-lib-link'>" + x.stair.toUpperCase() + " </a>";
                            
                        } else if (item.dataset.id == 'exit' && x.exit) {
                            item.innerHTML += "<a style='padding:.5em;font-size:1.3em' data-label='" + p + "' data-item='exit' class='item-lib-link'>" + x.exit.toUpperCase() + " </a>";
            ...
            closest_div.forEach((item) => {
                if (x.items.includes(item.dataset.id)) {
                    ...
                    // console.log("item", item);
                        item.innerHTML += "<a data-item='" + item.dataset.id + " ' data-label='" + p + "' class='item-closest-link'><img src='../"  +  app_version + "/skin/ak/hotspots/" + item.dataset.id + ".png' width='45' height='45' /></a>";
                }
            })
        }
        /*
        ...
        */
        ...
        for (var c in closest) {
            closest[c].onpointerup = function () {
                ...
                // sessionStorage.setItem("search_item", this.dataset.item);
                krpano.call('find_item("' + this.dataset.item.trim() + '")');
    ...
    function show_items_taketwo(scene, floor_id) {


        // stairs (named), closest items, items, exits (named)

        //accomp_container_hide();
        hide_overlay();
        current_type = "html";
        plugin.visible = true;

        let item_lib_content_taketwo = krpano.get('data[items_lib_twok].content');
        ...
        let close_but = document.getElementById("close_but");
        close_but.onpointerup = function () {
            //accomp_container_show();
            hide_overlay();
        }
        let items_div = document.querySelectorAll('.hcn-item-link-container');
        let closest_div = document.querySelectorAll('.item-closest-container');
        let ov = krpano.get('plugin[scene]');
        let o = ov.get_path_data(true), x;
        ...
        for (var p in o) {
            ...
            items_div.forEach((item) => {
                if (x.items.includes(item.dataset.id)) {
                    ...
                    item.innerHTML += "<a data-item='" + item.dataset.id + " ' data-label='" + p + "' class='item-lib-link'> "+item.dataset.id+" </a>";
